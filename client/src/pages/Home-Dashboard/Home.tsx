...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen rounded-2xl text-white p-6 space-y-8">
      {/* Header */}
      <header className="flex items-center gap-4">
        ...
        <div className="bg-darkbg border-4 border-neonblue p-4 shadow-lg rounded-none relative">
          <h1 className="text-md md:text-xl font-bold">
            Welcome Back, {user?.name || "Boo"}!
          </h1>
          <p className="text-gray-300 text-xs md:text-sm">
            Continue your PC building journey
          </p>

          <div className="absolute left-[-12px] top-6 w-3 h-3 bg-darkbg border-l-4 border-b-4 border-neonblue"></div>
        </div>
      </header>

      {/* Progress section */}
      <motion.section
        className="bg-darkbg border border-neonblue p-5 rounded-2xl shadow-md"
        whileHover={{
          y: -5,
        ...
        <div className="flex flex-wrap gap-6">
          {modes.map((mode, index) => (
            <Link key={index} to={mode.link}>
              <motion.div
                className="bg-darkbg border border-neonblue p-6 rounded-2xl shadow-md cursor-pointer w-64 h-64 flex flex-col justify-between"
                whileHover={{
                  y: -5,
                  boxShadow: "0px 4px 15px rgba(0,0,0,0.3)",
                }}
              >
                {/* Icon */}
                <div className="flex items-center justify-center w-14 h-14 rounded-[12px] bg-neonblue/20 text-neonblue mb-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-300 mt-1">{mode.desc}</p>
                </div>
      ...
      <section>
        ...
        <div className="flex flex-wrap gap-4 bg-darkgray rounded-2xl p-4">
          <motion.div
            className="bg-darkbg border border-neonblue p-4 rounded-xl shadow-md flex flex-col items-center justify-center w-[120px] h-[120px] text-center"
            whileHover={{ y: -5, boxShadow: "0px 4px 15px rgba(0,0,0,0.3)" }}
          >
            <div className="w-12 h-12 flex items-center justify-center rounded-full bg-neonblue/20 text-neonblue mb-2">
