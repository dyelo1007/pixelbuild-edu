...
async function hydratePuzzleMaps(puzzleDoc: any) {
  ...
  const lockedIds = Object.values(locked).filter(Boolean).map((id:any)=>id.toString());
  const solutionMap = puzzle.solution ? { ...puzzle.solution } : {};
  const solutionIds = Object.values(solutionMap).filter(Boolean).map((id:any)=>id.toString());
  const allIds = Array.from(new Set([...lockedIds, ...solutionIds]));
  const components = allIds.length ? await Component.find({ _id: { $in: allIds } }).lean() : [];
  const compById: Record<string, any> = Object.fromEntries(components.map((c:any)=>[c._id.toString(), c]));
  ...
  if (palette.length && typeof palette[0] === "string") {
    ...
    const paletteById = Object.fromEntries(paletteDocs.map((c:any)=>[c._id.toString(), c]));
    palette = palette.map((id:any)=> paletteById[id.toString()] ?? null);
  }

  return { ...puzzle, lockedComponents: hydratedLocked, solution: hydratedSolution, componentPalette: palette };
}
...
export const getVisiblePuzzles = async (req: Request, res: Response) => {
  try {
    const attemptedPuzzles = await PuzzleAttempt.find({ studentId: req.user?._id }).select("puzzleId");
    const attemptedPuzzleIds = attemptedPuzzles.map((a) => a.puzzleId);

    const rawPuzzles = await Puzzle.find({ visible: true, _id: { $nin: attemptedPuzzleIds } });
    const puzzles = await Promise.all(rawPuzzles.map((p: any) => hydratePuzzleMaps(p)));
    res.json(puzzles);
  } catch (err: any) {
...
export const submitPuzzleAttempt = async (req: Request, res: Response) => {
  try {
    ...
    const solutionObject: Record<string, string> = Object.fromEntries(
      Array.from(puzzle.solution.entries()).map(([slot, compId]) => [slot, compId.toString()])
    );

    let score = 0;
    Object.entries(build).forEach(([slot, componentId]) => {
      if (solutionObject[slot] && solutionObject[slot].toString() === componentId) {
        score += 50;
      }
    });

    const attempt = new PuzzleAttempt({ puzzleId, studentId, build, score, completed: true });
    await attempt.save();
    res.status(201).json({ message: "Attempt saved!", attempt });
  } catch (err: any) {
    if (err.code === 11000) {
      return res.status(400).json({ message: "You have already attempted this puzzle." });
    }
...
export const getPuzzleResultsForAdmin = async (req: Request, res: Response) => {
  try {
    ...
    const allStudents = await User.find({ role: "student" }).select("username email").lean();
    const attempts = await PuzzleAttempt.find({ puzzleId: req.params.id }).lean();
    const attemptsMap = new Map(attempts.map((a) => [a.studentId.toString(), a]));

    const results = allStudents.map((student) => {
      const attempt = attemptsMap.get(student._id.toString());
      return {
        student: { _id: student._id, name: student.username, email: student.email },
        taken: !!attempt,
  ...
  try {
    ...
    const lockedComponents: { [slot: string]: IComponent } = {};
      if (puzzle.lockedComponents) {
        for (const [slot, comp] of (puzzle.lockedComponents as unknown as Map<string, IComponent>).entries()) {
    ...
    const solution: { [slot: string]: IComponent } = {};
      if (puzzle.solution) {
        for (const [slot, comp] of (puzzle.solution as unknown as Map<string, IComponent>).entries()) {
          solution[slot] = comp;
        }
      }
    ...
    console.log("📤 Puzzle fetched for edit (hydrated):", JSON.stringify(hydratedPuzzle, null, 2));
...
export const updatePuzzle = async (req: Request, res: Response) => {
  try {
    ...
    if (updateData.lockedComponents) {
      updateData.lockedComponents = new Map(Object.entries(updateData.lockedComponents));
    }
    ...
    console.log("✅ Puzzle updated (hydrated):", JSON.stringify(hydrated, null, 2));
